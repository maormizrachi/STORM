...
int main()
{
    ...
    assert(state.isDDMC() && state.isResident() && state.hasPendingFlux());
    state.clearPendingFlux();
    assert(!state.hasPendingFlux());
    state.clearDDMC();
    assert(!state.isDDMC() && !state.isResident());

    double const conductance = STORM::ddmc::TwoSidedConductance(
        1.0, 0.5, 2.0, 0.5, 2.0);
    assert(std::abs(conductance - 2.0) < 1.0e-14);
    double const forward = STORM::ddmc::LeakageRate(
        1.0, 1.0, 0.5, 2.0, 0.5, 2.0);
    double const reverse = STORM::ddmc::LeakageRate(
        1.0, 1.0, 0.5, 2.0, 0.5, 2.0);
    assert(STORM::ddmc::ReciprocityResidual(1.0, forward, 1.0, reverse) == 0.0);
    ...
    double const totalMass = STORM::ddmc::PlanckBandMass(
        boundaries, 2.0, 0, 3);
    assert(lowMass > 0.0 && totalMass > lowMass);
    ...
    for(double mu : {1.0e-8, 1.0e-3, 0.1, 0.5, 0.99})
    {
        ...
        double const admission = STORM::ddmc::StaticAdmissionProbability(
            mu, 10.0, 0.5);
        assert(admission >= 0.0 && admission <= 1.0);
    }
