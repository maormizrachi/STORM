...
public:
    ...
    double computeDDMCFaceDiffusionCoefficient(const CellT &cell, double faceTemperature, std::size_t groupCutoff) const
    {
        ...
        {
            const double sigmaTotal = owner_.opacity_->CalcPlanckOpacityAtTemperature(cell, faceTemperature) +
                                        owner_.opacity_->CalcScatteringOpacityAtTemperature(cell, faceTemperature);
            return (sigmaTotal > 0.0 and std::isfinite(sigmaTotal))? owner_.lightSpeed() / (3.0 * sigmaTotal) : 0.0;
        }
        else
        {
            const GroupArray energyCenters = owner_.opacity_->getEnergyCenters(owner_.energyBoundaries_);
            ...
            {
                const double absorption = owner_.opacity_->CalcAbsorptionOpacityAtTemperature(cell, energyCenters[group], faceTemperature);
                const double scattering = owner_.opacity_->CalcScatteringOpacityAtTemperature(cell, energyCenters[group], faceTemperature);
                const double sigmaTotal = absorption + scattering;
                const double bandMass = ddmc::PlanckBandMass(owner_.energyBoundaries_, kT, group, group + 1);
                if(!(sigmaTotal > 0.0) or not std::isfinite(sigmaTotal) or not (bandMass >= 0.0) or not std::isfinite(bandMass))
                {
                    return 0.0;
                }
            ...
            return (totalBandMass > 0.0 and weightedInverseOpacity > 0.0)? (owner_.lightSpeed() / 3.0) * weightedInverseOpacity / totalBandMass : 0.0;
        }
    }
    ...
    void precomputeDDMCData()
    {
        ...
        for(std::size_t i = 0; i < Ncells; ++i)
        {
            ...
            if(owner_.parameters_.withMultigroupDDMC and owner_.parameters_.withMultigroupOpacity)
            {
                ...
                for(std::size_t g = 0; g < NumGroups; ++g)
                {
                    ...
                    if(not foundNonDiffusive &&
                        g < owner_.parameters_.ddmcMaxGroupCutoff &&
                        sigT_g * meanChordLength >= owner_.parameters_.ddmcMinCellOpticalDepth)
                    {
                        cutoff = g + 1;
                        totalBgDiff += Bg;
            ...
            if(data.eligible)
            {
                for(std::size_t faceIdx : owner_.componentGrid().GetCellFaces(i))
                {
                    ...
                    if(owner_.componentGrid().IsPointOutsideBox(next))
                    {
                        ...
                        else if(behavior != DDMCBoundaryFaceBehavior::ThermalSource)
                        {
        ...
        for(std::size_t i = 0; i < Ncells; ++i)
        {
            ...
            for(std::size_t faceIdx : owner_.componentGrid().GetCellFaces(i))
            {
                ...
                if(owner_.componentGrid().IsPointOutsideBox(nextCellIndex))
                {
                    ...
                    if(ScalarProd(
                            normal,
                            owner_.componentGrid().GetMeshPoint(nextCellIndex) -
                                cellCenter) < 0.0)
                    {
                        normal = -normal;
                    }
        ...
        auto refreshMixedFaceChannels = [this](DDMCCellData &data)
        {
            ...
            for(DDMCFaceLeak &face : data.faceLeaks)
            {
                ...
                if(face.kind == ddmc::FaceKind::Internal &&
                    face.ddmcRate > 0.0 && !targetEligible)
                {
    ...
    {
        (void)fullDt;
        if constexpr(not radiation_imc_detail::has_member_momentum<ExtensivesT>::value)
        {
            return;
        }
        else
        {
            if(owner_.parameters_.noHydroFeedback or not owner_.parameters_.withHydro or owner_.parameters_.diffusionPressureGradient)
            {
                return;
            }
            ...
            {
                if(i >= owner_.ddmcCellData_.size() or i >= owner_.ddmcFluxRhsIntegrated_.size())
                {
                    continue;
                }
                DDMCCellData const &data = owner_.ddmcCellData_[i];
                PointT const rhs = owner_.ddmcFluxRhsIntegrated_[i];
                if(!data.eligible or not (data.sigmaMomentum > 0.0) or not (data.faceAreaSum > 0.0) or not (fastabs(rhs) > 0.0))
                {
                    continue;
                }
                ...
                double const determinant = xx * (yy * zz - yz * yz) - xy * (xy * zz - yz * xz) + xz * (xy * yz - yy * xz);
    ...
    bool tryDDMCStep(MCParticle &particle, Functionality &functionality)
    {
        ...
        if(not particle.radiationState.isResident() and particle.radiationState.bypassCellID != std::numeric_limits<std::size_t>::max())
        {
        ...
        if(owner_.parameters_.withMultigroupDDMC and owner_.parameters_.withMultigroupOpacity)
        {
            if(data.groupCutoff == 0 or data.groupCutoff > NumGroups)
            {
                exitDDMCToTransport(true);
                ++owner_.ddmcFallbackCount_;
                return false;
            ...
            if constexpr(radiation_imc_detail::has_member_velocity<CellT>::value)
            {
                ...
                {
                    radiation_imc_detail::lorentzTransformToComoving<PointT>(frequencyProbe, owner_.cells_[cellIndex], owner_.lightSpeed());
                }
            }
        ...
        auto convertResidentToLab = [&]()
        {
            if constexpr(radiation_imc_detail::has_member_velocity<CellT>::value)
            {
                if(useComovingFrame)
                {
                    radiation_imc_detail::lorentzTransformToLab<PointT>(particle, owner_.cells_[cellIndex], owner_.lightSpeed());
                    owner_.clampFrequencyToBounds(particle.frequency);
                    particle.initialWeight = std::abs(particle.weight);
                }
            }
            ...
            data.velocityDivergence < 0.0)
        {
            ...
            double const cutoffFrequency = owner_.energyBoundaries_[data.groupCutoff];
            double const growthRate = -data.velocityDivergence / 3.0;
            if(frequency > 0.0 and frequency < cutoffFrequency and growthRate > 0.0)
            {
        ...
        {
            if(owner_.parameters_.withMultigroupDDMC && data.groupCutoff > 0 &&
        ...
        if(cutoffEvent)
        {
            particle.frequency = std::nextafter(owner_.energyBoundaries_[data.groupCutoff], std::numeric_limits<double>::max());
            particle.velocity = owner_.sampleRandomVelocity(owner_.cells_[cellIndex], particle);
            finalizePolarization(particle.velocity);
        ...
        if(censusEvent)
        {
            ...
            if(owner_.parameters_.withMultigroupOpacity)
            {
                ...
                if(owner_.parameters_.withMultigroupDDMC and data.groupCutoff > 0 and data.groupCutoff <= NumGroups)
                {
                    double const kT = units::k_boltz *
                        owner_.cells_[cellIndex].temperature;
                    double const bandMass = ddmc::PlanckBandMass(
                    ...
                    for(std::size_t group = 0; group < data.groupCutoff; ++group)
                    {
                        ...
                        if(remaining <= groupMass || group + 1 == data.groupCutoff)
                        {
                            ...
                            particle.frequency = owner_.opacity_->SampleThermalEnergyInGroup(
                                ...
                                owner_.energyBoundaries_);
                            double const upperBand = owner_.energyBoundaries_[data.groupCutoff];
                            particle.frequency = std::min(particle.frequency, std::nextafter(upperBand, owner_.energyBoundaries_[0]));
                ...
                {
                    particle.frequency = owner_.opacity_->GetThermalEnergy(owner_.cells_[cellIndex], owner_.randomUnitOpen(particle), owner_.energyBoundaries_);
                }
                owner_.clampFrequencyToBounds(particle.frequency);
            }
            particle.velocity = owner_.sampleRandomVelocity(owner_.cells_[cellIndex], particle);
        ...
        if(eventPick <= data.totalLeakRate)
        {
            ...
            {
                auto const sourceFace = owner_.postProcessExternalSourceFaceIndex_.find(chosen->faceIndex);
                if(sourceFace == owner_.postProcessExternalSourceFaceIndex_.end() ||
                    sourceFace->second >= owner_.postProcessExternalSources_.size())
                {
                    throw StormError("DDMC selected an external-source face without an installed source");
                }
                PostProcessExternalSource const &source =
                    owner_.postProcessExternalSources_[sourceFace->second];
                if(radiation_imc_detail::ddmcStableCellID(
                    ...
                    source.cellID)
                {
                    throw StormError("DDMC selected an external-source face from the wrong transport cell");
                }

                nOut = source.outwardNormal / std::max(fastabs(source.outwardNormal), std::numeric_limits<double>::min());
                ...
                if(owner_.parameters_.withMultigroupOpacity)
                {
                    particle.frequency = owner_.samplePostProcessExternalSourcePlanckFrequency(owner_.cells_[cellIndex]);
                    owner_.clampFrequencyToBounds(particle.frequency);
                }

                bool const leaveDDMCBand = owner_.parameters_.withMultigroupDDMC and
                                            data.groupCutoff < NumGroups and
                                            particle.frequency >= owner_.energyBoundaries_[data.groupCutoff];
                if(leaveDDMCBand)
                {
                    particle.velocity = owner_.lightSpeed() * owner_.samplePostProcessExternalSourceDirection(nOut, particle);
    #ifdef MONTECARLO_POLARIZATION
                    if(owner_.polarizationEnabled())
                    {
            ...
            bool const useDDMCChannel = chosen->ddmcRate > 0.0 and owner_.randomUnitOpen(particle) < chosen->ddmcRate / chosen->rate;
            ...
            if(!targetDDMC and owner_.parameters_.withMultigroupDDMC and owner_.parameters_.withMultigroupOpacity)
            {
                std::size_t beginGroup = 0;
                if(chosen->targetDDMCEligible and chosen->targetGroupCutoff > 0 and chosen->targetGroupCutoff < data.groupCutoff)
                {
                    beginGroup = chosen->targetGroupCutoff;
                }
                double const kT = units::k_boltz * owner_.cells_[cellIndex].temperature;
                double const bandMass = ddmc::PlanckBandMass(owner_.energyBoundaries_, kT, beginGroup, data.groupCutoff);
                if(bandMass > 0.0)
                {
                    double remaining = owner_.randomUnitOpen(particle) * bandMass;
                    for(std::size_t group = beginGroup; group < data.groupCutoff; ++group)
                    {
                        double const groupMass = ddmc::PlanckBandMass(owner_.energyBoundaries_, kT, group, group + 1);
                        if(remaining <= groupMass || group + 1 == data.groupCutoff)
                        {
                            double const localRandom = groupMass > 0.0
                                ? std::clamp(remaining / groupMass, 0.0, 1.0)
                                : owner_.randomUnitOpen(particle);
                            particle.frequency = owner_.opacity_->SampleThermalEnergyInGroup(owner_.cells_[cellIndex], group, localRandom, owner_.energyBoundaries_);
                            break;
                        }
                        remaining -= groupMass;
                    }
            ...
            if constexpr(radiation_imc_detail::has_member_velocity<CellT>::value)
            {
                if(owner_.parameters_.withHydro && !owner_.parameters_.MMC && !owner_.parameters_.staticScatterers && !targetDDMC)
                {
                    ...
                    radiation_imc_detail::lorentzTransformToLab<PointT>(transportParticle, sourceCell, owner_.lightSpeed());
            ...
            std::size_t const diagnosticGroup = (targetDDMC or not owner_.parameters_.withMultigroupOpacity)?
                DDMC_DIAGNOSTIC_GREY_GROUP : owner_.opacity_->findGroup(particle.frequency, owner_.energyBoundaries_);
            owner_.recordDDMCDiagnosticEvent(targetDDMC ? DDMCDiagnosticEventKind::DDMCToDDMC : DDMCDiagnosticEventKind::DDMCToIMC,
            ...
            if(targetDDMC)
            {
                ...
                {
                    owner_.addDDMCFluxContribution(chosen->nextCellIndex, fluxContribution);
                }
                else
                {
                    particle.radiationState.pendingFlux = fluxContribution;
                    particle.radiationState.set(RadiationTransportState<PointT>::PendingFlux);
                }
            }
            ...
            functionality.boundaryCrossing = owner_.componentGrid().IsPointOutsideBox(chosen->nextCellIndex);
            if(targetDDMC)
            {
                ++owner_.ddmcResidentLeakCount_;
                if(chosen->nextCellIndex >= owner_.componentGrid().GetPointNo())
        ...
        else
        {
            ...
            GroupArray const cumulativeOpacity = owner_.opacity_->GetCumulativeOpacity(cell, owner_.energyBoundaries_);
            double const opacityCdfCoordinate =
                ddmc::UpperBandOpacityCdfCoordinate(cumulativeOpacity, data.groupCutoff, owner_.randomUnitOpen(particle));
            if(!std::isfinite(opacityCdfCoordinate))
            {
                StormError eo("RadiationIMC DDMC upscatter has no opacity-weighted upper frequency band");
                eo.addEntry("Cell index", cellIndex);
                eo.addEntry("Group cutoff", data.groupCutoff);
                eo.addEntry("Total emission weight", cumulativeOpacity.back());
                double const weightThroughCutoff = (data.groupCutoff > 0 and data.groupCutoff <= cumulativeOpacity.size())? cumulativeOpacity[data.groupCutoff - 1] : 0.0;
                eo.addEntry("Emission weight through cutoff", weightThroughCutoff);
                throw eo;
            }
            particle.frequency = owner_.opacity_->GetThermalEnergy(
                cell, opacityCdfCoordinate, owner_.energyBoundaries_);
            owner_.clampFrequencyToBounds(particle.frequency);
    ...
    {
        if(not owner_.parameters_.withDDMC or not owner_.parameters_.ddmcInterfaceDiagnostics)
        {
            return;
        }
        ...
        DDMCDiagnosticEventKey const key{kind, faceIndex, sourceCellID, targetCellID, group};
        auto inserted = owner_.ddmcDiagnosticEvents_.emplace(key, DDMCDiagnosticEventAccumulator{});
        ...
        std::size_t faceIndex)
    {
        ...
        std::size_t const sourceGroupCutoff = (sourceCellIndex < owner_.ddmcPointGroupCutoff_.size())? owner_.ddmcPointGroupCutoff_[sourceCellIndex] : 0;
        std::size_t const targetGroupCutoff = (targetCellIndex < owner_.ddmcPointGroupCutoff_.size())? owner_.ddmcPointGroupCutoff_[targetCellIndex] : 0;
        std::size_t const diagnosticGroup = owner_.parameters_.withMultigroupOpacity? owner_.opacity_->findGroup(targetComoving.frequency, owner_.energyBoundaries_) : DDMC_DIAGNOSTIC_GREY_GROUP;
        ...
        if(owner_.parameters_.withMultigroupDDMC and owner_.parameters_.withMultigroupOpacity)
        {
            ...
            owner_.clampFrequencyToBounds(frequency);
            if(cutoff == 0 or cutoff > NumGroups or frequency >= owner_.energyBoundaries_[cutoff])
            {
                owner_.recordDDMCDiagnosticEvent(
        ...
        owner_.ddmcInterfaceMinimumMu_ = std::min(owner_.ddmcInterfaceMinimumMu_, mu);
        ...
        if constexpr(radiation_imc_detail::has_member_velocity<CellT>::value)
        {
            if(useVelocityFrames and owner_.parameters_.ddmcUseMovingInterfaceCorrection)
            {
                double const betaNormal = -ScalarProd(faceVelocity, normal) * owner_.inverseLightSpeed();
                if(not std::isfinite(betaNormal) or std::abs(betaNormal) > owner_.parameters_.ddmcMaxInterfaceVelocityOverC)
                {
                ...
                if(!(movingFactor > 0.0) or not std::isfinite(movingFactor) or movingFactor > owner_.parameters_.ddmcMaxMovingInterfaceWeightCorrection)
                {
                    ++owner_.ddmcInterfaceGuFallbackCount_;
                    particle.radiationState.bypassCellID = targetID;
                    return bypassMovingInterface();
        ...
        double admission = ddmc::StaticAdmissionProbability(mu, targetOpacity, targetDistance);
        if(targetCellIndex < owner_.ddmcPointSingleScatterAlbedo_.size())
        {
            const double coefficient = ddmc::Densmore2006CellCoefficient(targetOpacity, owner_.ddmcPointSingleScatterAlbedo_[targetCellIndex], targetDistance);
            if(ddmc::IsProbabilisticDensmore2006Coefficient(coefficient))
            {
    ...
    bool keepDDMCThermalBoundaryParticle(MCParticle &particle)
    {
        ...
        for(std::size_t faceIdx :
            owner_.componentGrid().GetCellFaces(cellIndex))
        {
            const std::pair<std::size_t, std::size_t> &neighbors =
                owner_.componentGrid().GetFaceNeighbors(faceIdx);
            std::size_t const nextCellIndex =
                neighbors.first == cellIndex
                ? neighbors.second : neighbors.first;
            if(!owner_.componentGrid().IsPointOutsideBox(nextCellIndex) ||
               owner_.componentBoundary()->getDDMCBoundaryFaceBehavior(
            ...
            double const cellDistance = std::abs(ScalarProd(
                owner_.componentGrid().FaceCM(faceIdx) - cellCenter, normal));
            double const planeDistance = std::abs(ScalarProd(
                particle.location - owner_.componentGrid().FaceCM(faceIdx),
                normal));
            double const faceScale = std::max(
                {cellDistance,
                 std::sqrt(owner_.componentGrid().GetArea(faceIdx)),
                 std::numeric_limits<double>::min()});
        ...
        double const mu = std::clamp(
            -ScalarProd(
                targetComoving.velocity / speed, outwardNormal),
            0.0, 1.0);
        DDMCCellData const &data = owner_.ddmcCellData_[cellIndex];
    ...
    std::string getDDMCFaceDiagnosticsTSV(double xMin, double xMax) const
    {
        ...
        for(std::size_t cellIndex = 0; cellIndex < owner_.ddmcCellData_.size(); ++cellIndex)
        {
            DDMCCellData const &data = owner_.ddmcCellData_[cellIndex];
